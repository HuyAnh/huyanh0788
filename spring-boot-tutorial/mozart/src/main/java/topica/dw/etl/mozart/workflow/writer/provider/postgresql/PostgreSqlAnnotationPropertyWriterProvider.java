 ...
 * @author trungnt9
 */
public interface PostgreSqlAnnotationPropertyWriterProvider<T> extends SqlWriterProvider<T> {

    final Logger LOG = LoggerFactory.getLogger(PostgreSqlAnnotationPropertyWriterProvider.class);
    final String ON_UPDATE_STATEMENT = "ON CONFLICT ";
    final String DO_UPDATE = "DO UPDATE SET ";
    ...
    default String extractSqlFromThis(Class<T> cls) throws UnsupportedGenerateSqlException {
        ...
        if (tableAnno != null) {
            ...
            if (!listColumn.isEmpty()) {
                ...
                if (!listPk.isEmpty()) {
                    builder.append(SPACE).append(ON_UPDATE_STATEMENT).append(LEFT_PARENTHESIS).append(listPk.stream()
                            .map(e -> e.getAnnotation(DWTableColumn.class).value()).collect(Collectors.joining(", ")))
                            .append(RIGHT_PARENTHESIS).append(SPACE).append(DO_UPDATE);
